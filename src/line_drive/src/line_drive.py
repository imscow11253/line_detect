...
#draw rectangle
def draw_rec(img, lpos, rpos, offset=0):
    center = (lpos + rpos) / 2
...
# get lpos, rpos
def get_line_info(lines, left=False, right=False):
    global width, height
    global offset, gap
...
def process_image(frame):
    ...
    left_lines, right_lines = divide_lr(all_lines)
    # get center of lines
    lx1, lx2, lpos = get_line_info(left_lines, left=True)
    rx1, rx2, rpos = get_line_info(right_lines, right=True)
    ...
    #draw rectangle
    # rect_center_x_left = (int(lx1) + int(lx2)) / 2
    # rect_center_y_left = (height + height//2) / 2
    # rect_center_x_right = (int(rx1) + int(rx2)) / 2
    # rect_center_y_right = (height + height//2) / 2
    # rect_center_x_center = rect_center_x_left + rect_center_x_right / 2
    # rect_center_y_center = rect_center_y_left + rect_center_y_right / 2
    
    # cv2.rectangle(frame, (rect_center_x_left - 5, rect_center_y_left - 5), (rect_center_x_left + 5, rect_center_y_left + 5), (0,255,0), -1)
    # cv2.rectangle(frame, (rect_center_x_right - 5, rect_center_y_right - 5), (rect_center_x_right + 5, rect_center_y_right + 5), (0,255,0), -1)
    # cv2.rectangle(frame, (rect_center_x_center - 5, rect_center_y_center - 5), (rect_center_x_center + 5, rect_center_y_center + 5), (0,255,0), -1)
    frame = draw_rec(frame, lpos, rpos, offset)
...
#     frame = draw_lines(frame, left_lines)
#     frame = draw_lines(frame, right_lines)
#     #frame = draw_rec(frame, left_lines, right_lines, offset)
#     #frame = cv2.line(frame, (230, 235), (410, 235), (255, 255, 255), 2)
...
def start():
    ...
    while True:
        ...
        angle = 320 - center
        steer_angle = (angle + 20)* 0.7 
        #angle_list.append(angle)
        #print(max(angle_list), ", " ,min(angle_list))
